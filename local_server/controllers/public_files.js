...
//public upload
    exports.upload_one_public_file = (req, res) =>{   //?save the public file(path, name, size, owner) in database
...
//return pub files
    exports.send_all_public_files = (req, res) => {  //?send all the public files(path, name, id_file, extension, owner)in a list
        let files_list = [];
        Public_files.find({})
        .cursor()
        .eachAsync(file => {
...
//public download
    exports.return_public_to_download = (req, res) => {     //?download the file on the computer of user
